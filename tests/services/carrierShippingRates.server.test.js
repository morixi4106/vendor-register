...
test("carrier shipping rates split physical items by their owning vendor store", async () => {
  ...
  assert.equal(result.requiresMarketplaceCheckout, true);
  assert.equal(result.governedProductCount, 1);
  assert.deepEqual(
...
test("carrier shipping rates do not expose standard checkout rates for governed marketplace products", async () => {
  let quoteCallCount = 0;
  const action = createCarrierShippingRatesAction({
    resolveCarrierFulfillmentOwnershipImpl: async ({ quoteRequest }) => ({
      ok: true,
      quoteRequest,
      requiresMarketplaceCheckout: true,
      governedProductCount: 1,
    }),
...
test("carrier shipping rates returns rates from Shipping V2 quote response", async () => {
  ...
  assert.equal(
    infoLogs.some(([message]) => message === "carrier shipping rates request:"),
    true,
  );
  assert.equal(
    infoLogs.some(
      ([message]) => message === "carrier shipping rates response:",
    ),
    true,
  );
  ...
  assert.equal(
    quoteResponseLog[1].shippingGroups[0].lines[0].variantId,
    "111222333",
  );
...
test("upsertShippingV2CarrierService updates an existing Shipping V2 carrier service", async () => {
  ...
  assert.equal(
    result.callbackUrl,
    "https://example.trycloudflare.com/carrier/shipping-rates",
  );
  assert.equal(
    calls[1].variables.input.id,
...
test("getCarrierCallbackUrl uses the current dev tunnel URL exactly", () => {
  assert.equal(
    getCarrierCallbackUrl(
      "https://low-alpine-hosts-contributed.trycloudflare.com",
    ),
...
test("upsertShippingV2CarrierService creates when no existing service is found", async () => {
  ...
  assert.equal(
    calls[1].variables.input.callbackUrl,
    "https://example.trycloudflare.com/carrier/shipping-rates",
  );
});
