...
- どちらの方式でも、決済予定上限以上の現在の未精算残高を別途確保すること
- 外部設定証跡と入金証跡の64桁SHA-256を保存すること

既存Payoutを使う場合も、今回の決済を銀行着金まで待つ場合も、全額返金できる
別の未精算残高を決済予定上限以上確保した証跡が必要です。入金証拠の照合後、
実際に返金する直前にも残高を再確認し、証跡参照とSHA-256を記録してください。
決済運用画面では、
