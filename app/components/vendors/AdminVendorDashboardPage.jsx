...
function badgeClass(text) {
  if (text === "対応要" || text === "在庫切れ" || text === "却下" || text === "保留") {
    return "dash-badge dash-badge-red";
  }
  if (text === "発送待ち" || text === "一部発送" || text === "在庫少" || text === "申請中" || text === "対応中") {
    return "dash-badge dash-badge-yellow";
  }
  if (text === "承認済み" || text === "販売中" || text === "発送済み") {
...
export default function VendorDashboard() {
  ...
  const filteredProducts = useMemo(() => {
    ...
    return products.filter(product => {
      return String(product.name || "").toLowerCase().includes(q) || String(product.sku || "").toLowerCase().includes(q) || String(product.tracking || "").toLowerCase().includes(q);
    });
  }, [query, products]);
  ...
  return <div className="vendor-dashboard">
      <style>{`
        .vendor-dashboard{
          min-height:100vh;
          background:#f3f4f6;
      ...
      <div className="dash-topbar">
        <div className="dash-topbar-inner">
          ...
          <div className="dash-search">
            <input type="text" placeholder="商品名・SKU・追跡番号で検索" value={query} onChange={e => setQuery(e.target.value)} />
          </div>
      ...
      <div className="dash-layout">
        ...
        <main className="dash-main">
          <section className="dash-cards">
            {summaryCards.map(card => <div className="dash-card" key={card.title}>
          ...
          <section className="dash-grid-2">
            <div className="dash-card">
              ...
              <div className="dash-chart">
                ...
                return <div className="dash-bar-wrap" key={item.label}>
              ...
              <div className="dash-health-list">
                <div className="dash-health-row">
                  <div className="dash-health-head">
                    ...
                    <span>
                      {priorityOrders.filter(order => order.countdownHours > 0 && order.countdownHours <= 24).length}件
                    </span>
                  </div>
                  <div className="dash-progress">
                    <div className="dash-progress-fill" style={{
                    width: `${Math.min(100, priorityOrders.length === 0 ? 0 : priorityOrders.filter(order => order.countdownHours > 0 && order.countdownHours <= 24).length / priorityOrders.length * 100)}%`
                  }} />
                  </div>
                </div>
                ...
                <div className="dash-health-row">
                  ...
                  <div className="dash-progress">
                    <div className="dash-progress-fill" style={{
                    width: `${Math.min(100, products.length === 0 ? 0 : products.filter(p => p.status === "在庫少").length / products.length * 100)}%`
                  }} />
                  </div>
                </div>
                ...
                <div className="dash-health-row">
                  ...
                  <div className="dash-progress">
                    <div className="dash-progress-fill" style={{
                    width: `${Math.min(100, products.length === 0 ? 0 : products.filter(p => p.status === "在庫切れ").length / products.length * 100)}%`
                  }} />
                  </div>
                </div>
              </div>
          ...
          <section className="dash-grid-3">
            <div className="dash-card">
              ...
              <div className="dash-order-list">
                {priorityOrders.map(order => <div className="dash-order-item" key={order.id}>
                    ...
                    <div className="dash-order-right">
                      ...
                      <div className={`dash-countdown-box ${order.countdownHours <= 24 && order.countdownHours > 0 ? "is-danger" : ""}`}>
                        {formatCountdown(order.countdownHours)}
                      </div>
                      <p className="dash-order-sub" style={{
                    marginTop: "8px"
                  }}>
                        {order.age}
                      </p>
            ...
            <div className="dash-card">
              ...
              <div className="dash-monthly-list">
                {monthlySales.map(item => <div className="dash-monthly-row" key={`${item.sku}-${item.name}`}>
                    <span>{item.name}</span>
