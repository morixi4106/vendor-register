...
function Metric({
  ...
  return <div className="preview-metric">
      <span>{label}</span>
      <strong>{value}</strong>
    </div>;
}
function StatusBadge({
  ...
  const label = product.deliveryEligibility?.label || STATUS_LABELS[status] || status;
  if (status === "AVAILABLE" || !label) {
    return null;
  }
  return <span className={`preview-status preview-status--${tone}`}>{label}</span>;
}
function CountryChips({
  ...
  return <span className={`preview-country-chips preview-country-chips--${tone}`}>
      {countries.map(country => <span className={`preview-country-chip preview-country-chip--${tone}`} key={country.code}>
          {country.label}
        </span>)}
    </span>;
}
function getDeliveryRestrictionCountries(summary) {
  ...
  if (summary?.hasAllowedCountryLimit) {
    const allowedCountryCodes = new Set((summary.allowedCountries || []).map(country => country.code));
...
function DeliveryRestrictionButton({
  product,
  onOpen
}) {
  ...
  return <button className="preview-restriction-button" type="button" onClick={() => onOpen(product)}>
      購入できない配送先を見る
    </button>;
}
function DeliveryRestrictionModal({
  product,
  onClose
}) {
  const summary = product?.deliveryRestrictionSummary;
  ...
  return <div className="preview-modal-backdrop" role="presentation" tabIndex={-1} onClick={onClose} onKeyDown={event => {
    if (event.key === "Escape") {
      onClose();
    }
  }}>
      <section className="preview-modal" role="dialog" aria-modal="true" aria-labelledby="preview-delivery-modal-title" onClick={event => event.stopPropagation()}>
        <div className="preview-modal-header">
          <h2 id="preview-delivery-modal-title">{product.name}</h2>
          <button className="preview-modal-close" type="button" aria-label="閉じる" onClick={onClose}>
            x
          </button>
        </div>
        ...
        <div className="preview-restriction-card preview-restriction-card--blocked">
          ...
          <CountryChips countries={unavailableCountries} emptyText="購入できない配送先は登録されていません。" />
        </div>

        {summary.warningCountries?.length ? <div className="preview-restriction-card preview-restriction-card--warning">
            <div className="preview-restriction-heading">
...
export default function PublicVendorPreviewPage() {
  ...
  const [checkoutProductId, setCheckoutProductId] = useState(data.products[0]?.id || "");
  const [useSalesCredit, setUseSalesCredit] = useState(true);
  const [salesCreditAmount, setSalesCreditAmount] = useState("100");
  const activeRestrictionProduct = data.products.find(product => product.id === restrictionProductId) || null;
  const checkoutProduct = data.products.find(product => product.id === checkoutProductId) || data.products[0] || null;
  const checkoutProductPrice = Number(checkoutProduct?.price || 0);
  const checkoutSalesCreditAmount = useSalesCredit ? Math.min(checkoutProductPrice, parsePositiveAmount(salesCreditAmount)) : 0;
  const checkoutPayAmount = Math.max(0, checkoutProductPrice - checkoutSalesCreditAmount);
  return <main className="preview-page">
      ...
      <div className="preview-shell">
        <header className="preview-header">
          <div>
            ...
            <div className="preview-meta">
              ...
              {data.store.country ? <span className="preview-chip">{data.store.country}</span> : null}
              {data.store.category ? <span className="preview-chip">{data.store.category}</span> : null}
              {hasDeliveryCountry ? <span className="preview-chip">
                  配送先: {data.deliveryCountry}
                </span> : <span className="preview-chip">配送先未選択</span>}
            </div>
          </div>
        ...
        <section className="preview-panel">
          <Form className="preview-controls" method="get">
            <label className="preview-field">
              配送先国
              <select className="preview-select" name="deliveryCountry" defaultValue={data.deliveryCountry || ""}>
                {COUNTRY_OPTIONS.map(country => <option key={country.value || "none"} value={country.value}>
                    {country.label}
            ...
            <label className="preview-check">
              <input type="checkbox" name="filterEligible" value="1" defaultChecked={data.filterEligible} />
              販売できない商品を非表示
            </label>
        ...
        {checkoutProduct ? <section className="preview-panel">
            <Form className="preview-checkout" method="post">
              <input type="hidden" name={`quantity:${checkoutProduct.id}`} value="1" />
              ...
              <div className="preview-checkout-grid">
                <label className="preview-field">
                  商品
                  <select className="preview-select" value={checkoutProduct.id} onChange={event => {
                ...
                <label className="preview-field">
                  購入者メール
                  <input className="preview-input" name="email" type="email" placeholder="seller@example.com" required />
                </label>

                <label className="preview-field">
                  売上金充当
                  <input className="preview-input" name="salesCreditAmount" type="number" min="1" max={checkoutProductPrice || undefined} value={salesCreditAmount} disabled={!useSalesCredit} onChange={event => {
                ...
                setSalesCreditAmount(String(checkoutProductPrice ? Math.min(parsedAmount, checkoutProductPrice) : parsedAmount));
              }} />
                </label>

                <label className="preview-check">
                  <input type="checkbox" name="useSalesCredit" value="on" checked={useSalesCredit} onChange={event => setUseSalesCredit(event.target.checked)} />
                  売上金を使う
                </label>
              </div>
              ...
              <div className="preview-checkout-grid preview-checkout-grid--address">
                <label className="preview-field">
                  名
                  <input className="preview-input" name="firstName" defaultValue="Taro" required />
                </label>
                <label className="preview-field">
                  姓
                  <input className="preview-input" name="lastName" defaultValue="Yamada" required />
                </label>
                <label className="preview-field">
                  電話番号
                  <input className="preview-input" name="phone" defaultValue="09012345678" required />
                </label>
                <label className="preview-field preview-checkout-full">
                  住所
                  <input className="preview-input" name="address1" defaultValue="1-2-3 Jingumae" required />
                </label>
                <label className="preview-field">
                  市区町村
                  <input className="preview-input" name="city" defaultValue="Shibuya" required />
                </label>
                <label className="preview-field">
                  都道府県
                  <input className="preview-input" name="province" defaultValue="Tokyo" required />
                </label>
                <label className="preview-field">
                  郵便番号
                  <input className="preview-input" name="postalCode" defaultValue="150-0001" required />
                </label>
                <label className="preview-field">
                  国
                  <select className="preview-select" name="country" defaultValue={data.deliveryCountry || "JP"}>
                    {COUNTRY_OPTIONS.filter(country => country.value).map(country => <option key={country.value} value={country.value}>
        ...
        <section className="preview-products">
          ...
          return <article className={`preview-product preview-product--${tone}`} key={product.id}>
                {product.imageUrl ? <img className="preview-image" src={product.imageUrl} alt={product.name} /> : <div className="preview-image-placeholder">NO IMAGE</div>}
                ...
                <div className="preview-product-body">
                  ...
                  {product.description ? <p className="preview-description">{product.description}</p> : null}

                  {shouldShowStatusRow ? <div className="preview-status-row">
                      {shouldShowStatus ? <StatusBadge product={product} /> : null}
                      {product.category ? <span className="preview-status preview-status--info">
                          {product.category}
                        </span> : null}
                    </div> : null}

                  {product.deliveryEligibility?.message ? <p className={`preview-message preview-message--${tone}`}>
                      {product.deliveryEligibility.message}
                    </p> : null}

                  {!hasDeliveryCountry ? <DeliveryRestrictionButton product={product} onOpen={selectedProduct => setRestrictionProductId(selectedProduct.id)} /> : null}
                </div>
              </article>;
        })}
        </section>

        {data.products.length === 0 ? <div className="preview-empty">
            {data.hiddenProductCount > 0 ? "現在の配送先では表示できる商品がありません。フィルターを外すと販売不可理由を確認できます。" : "公開対象の商品がまだありません。"}
          </div> : null}
      ...
      <DeliveryRestrictionModal product={activeRestrictionProduct} onClose={() => setRestrictionProductId(null)} />
    </main>;
}
