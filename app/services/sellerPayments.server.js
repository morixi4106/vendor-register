...
const LEDGER_REPAIR_SCOPE_NEGATIVE_PAYOUTABLE_BALANCE =
  "negative_payoutable_balance";
const LEDGER_REPAIR_SCOPE_SHOPIFY_ORDER_REVERSAL_OVERAGE =
...
function isMultiSellerShopifyDisputeSettlementEnabled(env = process.env) {
  return (
    normalizeLowercase(env.MULTI_SELLER_SHOPIFY_DISPUTE_SETTLEMENT_ENABLED) ===
    "true"
  );
}
...
function allocateAmountByWeight(totalAmount, weightedItems) {
  ...
  const items = Array.isArray(weightedItems)
...
function getShopifyOrderOccurredAt(payload) {
  const value =
    payload?.created_at || payload?.processed_at || payload?.updated_at || null;
...
async function quarantinePaidShopifyOrder(
  ...
  if (prismaClient?.marketplaceOperationalCase?.findFirst) {
    operationalCase =
      (await prismaClient.marketplaceOperationalCase.findFirst({
        where: {
          marketplaceOrderId,
          caseType: "COMPLIANCE",
          status: { notIn: ["RESOLVED", "CLOSED"] },
    ...
    const created = await createMarketplaceOperationalCase(
      {
        ...
        currencyCode,
        summary:
          "Paid Shopify order requires sale eligibility review before settlement or shipment.",
        detailsJson: {
          shopDomain,
          shopifyOrderId,
          shopifyOrderName,
        ...
        assignedTo:
          normalizeText(env.SALE_ELIGIBILITY_QUARANTINE_ASSIGNEE) ||
          normalizeText(env.ADMIN_EMAIL) ||
          "INCIDENT_COMMANDER",
      },
      {
        prismaClient,
  ...
  const shopifyQuarantine = await applyShopifyOrderQuarantine(
    {
      ...
      requiresShipping: matchedLines.some(
        ({ lineItem }) => lineItem?.requires_shipping !== false,
      ),
    },
    {
  ...
  if (operationalCase && prismaClient?.marketplaceOperationalCase?.update) {
    const existingDetails =
      operationalCase.detailsJson &&
      typeof operationalCase.detailsJson === "object" &&
      !Array.isArray(operationalCase.detailsJson)
        ? operationalCase.detailsJson
        : {};
    ...
    if (prismaClient?.marketplaceOperationalCaseEvent?.create) {
      await prismaClient.marketplaceOperationalCaseEvent.create({
        data: {
          ...
          eventType:
            shopifyQuarantine.ok === true
              ? "SHOPIFY_QUARANTINE_APPLIED"
              : "SHOPIFY_QUARANTINE_PARTIAL_FAILURE",
          actor: "system:orders_paid_sale_eligibility",
          note:
            shopifyQuarantine.ok === true
              ? "Shopify fulfillment holds and quarantine tag were applied."
              : "Shopify order quarantine requires immediate manual completion.",
          metadataJson: {
            quarantineStatus: shopifyQuarantine.status,
            fulfillmentOrderCount:
              shopifyQuarantine.fulfillmentOrders?.length || 0,
            failedFulfillmentOrderCount: (
              shopifyQuarantine.fulfillmentOrders || []
            ).filter((entry) => entry?.ok !== true).length,
  ...
  if (
    prismaClient?.sellerOrder?.update &&
    Array.isArray(sellerOrderShadow?.sellerOrders)
  ) {
    for (const sellerOrder of sellerOrderShadow.sellerOrders) {
      const metadata =
        ...
        !Array.isArray(sellerOrder.metadataJson)
          ...
          : {};
      const currentReview =
        metadata.saleEligibilityReview &&
        typeof metadata.saleEligibilityReview === "object" &&
        !Array.isArray(metadata.saleEligibilityReview)
          ? metadata.saleEligibilityReview
          : {};
...
export async function processShopifyOrderPaidSettlement(
  ...
  const paymentRisk = integrityOnly
    ? null
    : await resolveShopifyOrderSalesCreditPaymentRisk(
        {
          payload,
          shopDomain,
          shopifyOrderId,
  ...
  const postOrderEligibilityEnabled =
    String(
      env.POST_ORDER_SALE_ELIGIBILITY_ENFORCEMENT_ENABLED ??
        process.env.POST_ORDER_SALE_ELIGIBILITY_ENFORCEMENT_ENABLED ??
        "true",
    )
      .trim()
      .toLowerCase() !== "false";
  if (postOrderEligibilityEnabled) {
    ...
    const failures = [
      ...(Array.isArray(eligibility?.failures) ? eligibility.failures : []),
      ...unmatchedProductIds.filter(Boolean).map((productId) => ({
        productId,
      ...
      return quarantinePaidShopifyOrder(
        {
          ...
          sellerIds,
          failures,
          evidence: Array.isArray(eligibility?.evidence)
...
export async function processShopifyDisputeSettlement(
  ...
  const multiSellerDisputeSettlementEnabled =
    isMultiSellerShopifyDisputeSettlementEnabled(env);
  ...
  if (sellerIds.length > 1) {
    ...
    if (SHOPIFY_DISPUTE_RELEASE_STATUSES.has(disputeStatus)) {
      const existingReleaseEntries = orderLedgerEntries.filter(
        (entry) =>
          entry?.entryType === "dispute_funds_reinstated" &&
          entry?.stripeObjectId === shopifyDisputeId,
      );
      ...
      const sellerRiskSummaries = sellerIds.map((sellerId) => ({
        ...
        riskSummary: summarizeShopifyOrderRiskLedgerEntries(
          orderLedgerEntries,
          sellerId,
        ),
      }));
      const totalHeldAmount = sellerRiskSummaries.reduce(
        (total, sellerRiskSummary) =>
          total + sellerRiskSummary.riskSummary.disputeHeldAmount,
        0,
      );
      const targetReleaseAmount = Math.min(
        requestedDisputeAmount || totalHeldAmount,
        totalHeldAmount,
      );
      ...
      const allocations = allocateAmountByWeight(
        targetReleaseAmount,
        sellerRiskSummaries.map((sellerRiskSummary) => ({
          ...sellerRiskSummary,
      ...
      return runInTransaction(prismaClient, async (tx) => {
          ...
          const ledgerEntry = await createLedgerEntry(
            {
              ...
              sellerStripeAccountId:
                allocation.paidEntry?.sellerStripeAccountId || null,
              ...
              metadataJson: {
                ...
                sellerHeldAmountBeforeRelease:
                  allocation.riskSummary.disputeHeldAmount,
              },
              occurredAt,
            },
            { prismaClient: tx },
        ...
        return {
          ...
          ledgerEntries,
          sellerIds,
          amount: ledgerEntries.reduce(
            (total, entry) => total + clampInteger(entry?.amount),
            0,
          ),
          currencyCode,
    ...
    const existingHoldEntries = orderLedgerEntries.filter(
      (entry) =>
        entry?.entryType === "dispute_created" &&
        entry?.stripeObjectId === shopifyDisputeId,
    );
    ...
    const totalHoldableAmount = sellerRiskSummaries.reduce(
      (total, sellerRiskSummary) =>
        total + sellerRiskSummary.riskSummary.remainingHoldableAmount,
      0,
    );
    const targetHoldAmount = Math.min(
      requestedDisputeAmount || totalHoldableAmount,
      totalHoldableAmount,
    );
        ...
        const ledgerEntry = await createLedgerEntry(
          {
            ...
            metadataJson: {
              ...
              sellerHoldableAmountBeforeDispute:
                allocation.riskSummary.remainingHoldableAmount,
            },
            occurredAt,
          },
          { prismaClient: tx },
  ...
  const sellerId = sellerIds[0];
  const singleSellerPaidEntry = paidEntries.find(
    (entry) => entry?.sellerId === sellerId,
  );
...
function getLedgerEntryShopifyOrderId(entry) {
  ...
  if (
    entry?.entryType === "shopify_order_paid" ||
    entry?.entryType === "shopify_order_cancelled"
  ) {
    return (
      normalizeShopifyGid("Order", entry?.stripeObjectId) ||
      normalizeText(entry?.stripeObjectId)
    );
  }
...
function buildSellerOrderReversalOverageRepairCandidates(seller, currencyCode) {
  ...
  for (const entry of Array.isArray(seller?.ledgerEntries)
    ...
    if (entry.entryType === "shopify_order_paid") {
      ...
      entry.entryType === "ledger_adjustment" &&
      getLedgerEntryRepairScope(entry) ===
        LEDGER_REPAIR_SCOPE_SHOPIFY_ORDER_REVERSAL_OVERAGE
    ) {
  ...
  return Array.from(groups.values())
    .map((group) => {
      const repairAmount =
        group.reversedAmount - group.paidAmount - group.repairedAmount;
...
export async function listSellerLedgerRepairCandidates(
  ...
  return sellers.flatMap((seller) => {
    ...
    const orderRepairCandidates =
      buildSellerOrderReversalOverageRepairCandidates(
        seller,
        normalizedCurrency,
      );
    const orderRepairTotal = orderRepairCandidates.reduce(
      (total, candidate) => total + clampInteger(candidate.repairAmount),
      0,
    );
    const remainingNegativeBalance = Math.min(
      0,
      payoutableLedgerBalance + orderRepairTotal,
    );
  ...
  return runInTransaction(prismaClient, async (tx) => {
    ...
    if (
      ...
      const paidAmount = orderEntries
        .filter((entry) => entry?.entryType === "shopify_order_paid")
        .reduce((total, entry) => total + clampInteger(entry?.amount), 0);
      const reversedAmount = orderEntries
        .filter((entry) =>
          SHOPIFY_ORDER_REVERSAL_ENTRY_TYPES.includes(entry?.entryType),
        )
        .reduce((total, entry) => total + clampInteger(entry?.amount), 0);
      const repairedAmount = orderEntries
        .filter(
          (entry) =>
            entry?.entryType === "ledger_adjustment" &&
            getLedgerEntryRepairScope(entry) ===
    ...
    return {
      ...
      currencyCode: normalizedCurrency,
      previousPayoutableLedgerBalance: payoutableLedgerBalance,
      nextPayoutableLedgerBalance:
        normalizedRepairScope ===
        LEDGER_REPAIR_SCOPE_NEGATIVE_PAYOUTABLE_BALANCE
          ? 0
          : payoutableLedgerBalance + repairAmount,
    };
  });
}
...
export async function approvePayoutRun(
  ...
  try {
    return await runSerializableTransaction(prismaClient, async (tx) => {
      ...
      const approvedTransferMethod =
        payoutRun.transferMethod || "stripe_connect_payout";
...
async function claimPayoutRunForExecution(
  ...
  return runSerializableTransaction(prismaClient, async (tx) => {
    ...
    if (
      ...
      !payoutRun.seller.stripeAccount?.payoutsEnabled
    ) {
...
export async function executePayoutRun(
  ...
  const balanceStripeClient =
    stripeClient ||
    (createPayout === createConnectedAccountPayout ? getStripeClient() : null);
  ...
  if (
    ...
    availableBalance < payoutCandidate.amount
  ) {
    return {
      ok: false,
